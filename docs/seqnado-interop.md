...
The restriction is enforced before DAG creation, so it holds whether a config is
generated by `regulonado config` or written by hand. With several projects
configured, `scaling.method: seqnado` is always an error; naming one project does
not make its factors comparable with tracks from the others.
