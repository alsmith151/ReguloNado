...
**Problem.** `polars` has no product or test imports. `pysam`, `psutil`, `lightning`, `plotly`, and `seaborn` have no production imports. `tangermeme` is deliberately not used by the attribution implementation. `bioframe` is used only to read a BED-like table. Loguru and standard-library logging coexist, with repeated `basicConfig` calls in CLI functions. `peft`, previously unused, is now load-bearing: `model/peft_adapters.py` imports it for LoRA/LoCon parameter-efficient fine-tuning of the backbone.
...
| `lightning` direct train dep | No identified product import | Remove unless clean-install tests prove an indirect runtime need | Hugging Face Trainer / current custom transfer code | Direct dependencies should describe direct contracts, not possible transitive needs | Medium | import search and training implementation |
| `peft` direct train dep | Now load-bearing | **Keep** | `model/peft_adapters.py` (LoRA/LoCon adapters) | Direct import for parameter-efficient fine-tuning of the backbone | None | `python/regulonado/model/peft_adapters.py` |
