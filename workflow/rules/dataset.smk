...
rule build_dataset:
    ...
    params:
        ...
        drop_missing=lambda w: (
            "--drop-missing" if config["build"]["drop_missing"] else "--no-drop-missing"
        ),
        dedupe=config["build"]["dedupe_tracks"],
    output:
        info=str(DATASET_DIR / "dataset_info.json"),
...
rule recompress_dataset:
    ...
    output:
        info=str(RECHUNK_DIR / "dataset_info.json"),
